...
@Composable
fun RegistrationScreenUsername(viewModel: AuthViewModel, onClickNextStep: () -> Unit, onClickBack: () -> Unit) {
    ...
    Box(modifier = Modifier.background(color = Black22).fillMaxSize()) {
        ...
        Box(modifier = Modifier.fillMaxSize().imePadding(), contentAlignment = Alignment.Center) {
            AuthComponent {
                ...
                RegistrationLineComponent(iconRegScreen = REGISTRATION_USERNAME)
            }
        }
