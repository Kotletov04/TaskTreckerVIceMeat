...
@Composable
fun RegistrationScreenUsername(
...
@Composable
private fun RegistrationMainUsername(
    ...
    Box(modifier = Modifier.background(color = MaterialTheme.colorScheme.primary).fillMaxSize()) {
        ...
        Box(modifier = Modifier.fillMaxSize().imePadding(), contentAlignment = Alignment.Center) {
            AuthComponent {
                ...
                UsernameTextField(viewModel = viewModel, textFieldIsActive = textFieldIsActive, focusRequester = focusRequester)
                ...
                RegistrationLineComponent(iconRegScreen = painterResource(MeatViceIcons.Person), stage = 3)
            }
        }
...
@Composable
private fun UsernameTextField(viewModel: AuthViewModel, textFieldIsActive: MutableState<Boolean>, focusRequester: FocusRequester) {
    TextField(
        modifier = Modifier
            .padding(10.dp)
        ...
        onValueChange = { newText ->
            viewModel.username.value = newText
        },
        placeholder = {
            Text(
                text = "Ваш ник",
